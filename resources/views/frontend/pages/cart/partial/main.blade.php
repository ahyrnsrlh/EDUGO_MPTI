                            ...
                            <tr>
                                ...
                                <td>
                                    <ul class="generic-list-item font-weight-semi-bold">
                                        <li class="text-black lh-18">Rp {{ number_format($item->course->discount_price * 15000, 0, ',', '.') }}</li>
                                        <li class="before-price lh-18">Rp {{ number_format($item->course->selling_price * 15000, 0, ',', '.') }}</li>
                                    </ul>
                                </td>
                ...
                <div class="bg-gray p-4 rounded-rounded mt-40px">
                    ...
                    <ul class="generic-list-item pb-4">
                        <li class="d-flex align-items-center justify-content-between font-weight-semi-bold">
                            ...
                            <span id="subTotalValue">Rp {{ number_format($subTotal * 15000, 0, ',', '.') }}</span> <!-- Update ID for easy targeting -->
                        </li>
                    ...
                    @if (session()->get('coupon'))
                        <li
                            ...
                            <span class="text-black">Total Discount:</span>
                            <span id="totalDiscount">
                                Rp {{ number_format(session()->get('coupon') * 15000, 0, ',', '.') }}
                            </span>
                        </li>
                    @endif
                        ...
                        <li class="d-flex align-items-center justify-content-between font-weight-semi-bold">
                            ...
                            <span id="totalAmount">
                                @if (session()->get('coupon'))
                                    Rp {{ number_format(($subTotal - session()->get('coupon')) * 15000, 0, ',', '.') }}
                                @else
        ...
        $('#applyCouponBtn').click(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#totalDiscount').text(
                        `Rp ${(totalDiscount * 15000).toLocaleString('id-ID')}`); // Show the total discount amount
                    ...
                    $('#totalAmount').text(
                        `Rp ${(totalAmount * 15000).toLocaleString('id-ID')}`); // Show the updated total
